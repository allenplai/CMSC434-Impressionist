    ...
    public boolean onMenuItemClick(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.menuLineSplatter:
                ...
                return true;
            case R.id.menuParticleEmitter:
                Toast.makeText(this, "Particle Emitter Brush", Toast.LENGTH_SHORT).show();
                _impressionistView.setBrushType(BrushType.ParticleEmitter);
                return true;
        }
        return false;
    }
        ...
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int whichButton) {
                        ...
                        // save the impressionist art to gallery
                        _impressionistView.setDrawingCacheEnabled(true);
                        Bitmap drawingBitmap = _impressionistView._offScreenBitmap;

                        // http://stackoverflow.com/questions/5126559/android-create-unique-string-for-file-name
//                        https://developer.android.com/reference/java/util/UUID.html
                        String fileName = UUID.randomUUID().toString() + ".png";
                        File file = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES), fileName);
                        try {
                            drawingBitmap.compress(Bitmap.CompressFormat.PNG, 100, new FileOutputStream(file));
                            FileUtils.addImageToGallery(file.getAbsolutePath(), getApplicationContext());
